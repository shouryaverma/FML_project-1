...
from .callbacks import state_to_features, ACTIONS, DenseNetwork
...
def setup_training(self):
    ...
    self.optimizer = optim.Adam(self.Dense_Q_Agent.parameters(), lr=0.0001)
    self.sl1_loss = nn.MSELoss().to(processing_unit)
    ...
    self.target_Dense_Q_Agent = DenseNetwork().to(processing_unit)
    self.target_Dense_Q_Agent.double()
    self.target_Dense_Q_Agent.load_state_dict(self.Dense_Q_Agent.state_dict())
    self.target_Dense_Q_Agent.eval()
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if old_game_state == None:
        ...
        if len(self.store_list)>1:
            ...
            pred_value = self.Dense_Q_Agent(previous_batch).gather(1,batch_action.view(-1,1))
            true_value = batch_reward + (self.gamma * mask * self.target_Dense_Q_Agent(current_batch).detach().max(1)[0]).unsqueeze(1)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    torch.save(self.Dense_Q_Agent.state_dict(),path+"\model_parameters.pt")
    ...
    self.scheduler.step(self.loss_total1[-1])
